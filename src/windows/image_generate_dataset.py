...
# 输出yolo格式的数据集到output_yolo文件夹
IMAGES_OUT_PATH = IMAGE_DATA_PATH + DATA_NAME + "/output_yolo/images/"
...
IMAGES_HAND_TEST = IMAGE_DATA_PATH + "hands/output_yolo/images/test/"
LABELS_HAND_TEST = IMAGE_DATA_PATH + "hands/output_yolo/labels/test/"

IMAGES_HAND_TRAIN = IMAGE_DATA_PATH + "hands/output_yolo/images/train/"
LABELS_HAND_TRAIN = IMAGE_DATA_PATH + "hands/output_yolo/labels/train/"

IMAGES_HAND_VAL = IMAGE_DATA_PATH + "hands/output_yolo/images/val/"
LABELS_HAND_VAL = IMAGE_DATA_PATH + "hands/output_yolo/labels/val/"
...
class YoloDataProducer(object):
    def __init__(
        self,
        background_file=BACKGROUND_INPUT_PATH,
        foreground_file=FOREGROUND_INPUT_PATH,
        per_num=1,
        overlap_factor=0.8,
        max_num=5,
        is_circle=True,
        ...
        self.num_to_file_map = {
            1: [IMAGES_OUT_PATH_TRAIN, LABELS_OUT_PATH_TRAIN],
    ...
    def augmente(self):
        ...
        if self.is_circle:
            p.rotate_without_crop(
                probability=0.6, max_left_rotation=45, max_right_rotation=45
            )
        # 类似的数据增强操作
        ...
        for filename in os.listdir(self.background_file + "output"):
            self.background_list.append(
                os.path.join(self.background_file + "output", filename)
            )
        # 收集增强后的前景图像路径并记录类别映射
        ...
        for key in sorted_map.keys():
            if key != i:
                messagebox.showerror(
                    "错误",
                    f"数据集的序号没有按照从0开始的严格自增的规则，请修改序号为 {key} 的图片",
                    parent=self.root,
                )
                return False
            i += 1
    ...
    def produce(self):
        ...
        while self.end_flag == False:
            show_progress(
                self.root,
                self.progressbar,
                (self.produce_count - last_num) * 80 / (step - 1),
            )
    ...
    def write_yolo_yaml(self):
        if len(self.class_map) > 0:
            ...
            data["path"] = (
                IMAGE_DATA_PATH.replace(ROOT, "", 1) + DATA_NAME + "/output_yolo"
            )  # code
        ...
        for _class in self.foreground_list_map.keys():
            for j in range(random.randint(1, self.max_num)):
                random_index = random.randint(
                    0, len(self.foreground_list_map[_class]) - 1
                )
    ...
    def random_point(self, img_background, img_foreground, file_name, point_list):
        ...
        while not is_success:
            ...
            for x1, y1, rows1, cols1, file_name1, _, _ in point_list:
                if x1 < x and y1 < y:
                    if (x - x1) < (rows1 * self.overlap_factor) and (y - y1) < (
                        cols1 * self.overlap_factor
                    ):
                        b_has = True
                        break
                elif x1 > x and y1 < y:
                    if (x1 - x) < (rows_f * self.overlap_factor) and (y - y1) < (
                        cols1 * self.overlap_factor
                    ):
                        b_has = True
                        break
                elif x1 < x and y1 > y:
                    if (x - x1) < (rows1 * self.overlap_factor) and (y1 - y) < (
                        cols_f * self.overlap_factor
                    ):
                        b_has = True
                        break
                else:
                    if (x1 - x) < (rows_f * self.overlap_factor) and (y1 - y) < (
                        cols_f * self.overlap_factor
                    ):
                        b_has = True
                        break
...
def run(per_num, overlap_factor, max_num, is_circle, root=None, progressbar=None):
    ...
    if producer.augmente():
        producer.produce()
        # producer.add_hand_data()  # 添加手部数据到数据集中，这通常用于训练模型识别手部
        producer.write_yolo_yaml()  # 生成YOLO训练所需的配置文件(yaml)，包含类别数量、类别名称等信息
