...
def device_count():
    ...
    try:
        cmd = "nvidia-smi -L | wc -len_params"
        return int(
...
def select_device(device="", batch_size=0, newline=True):
    ...
    if device == "cpu":
        ...
        ), "MPS not available. Install PyTorch >= 1.12 on MacOS."
        final_device = torch.device("mps")
        s += "MPS\n"
    elif device:  # CUDA device(s)
        os.environ["CUDA_VISIBLE_DEVICES"] = device
        assert (
            torch.cuda.is_available()
        ), f"CUDA not available, invalid '--device {device}' requested"

        devices = device.split(",")
        n = len(devices)
        assert (
            torch.cuda.device_count() >= n
        ), f"Not enough CUDA devices, {torch.cuda.device_count()} available, {n} requested"
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                ...
                mem = (
                    torch.cuda.memory_reserved() / 1e9 if device.type == "cuda" else 0
                )  # (GB)
