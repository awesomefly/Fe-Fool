...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(
        ...
        for i, (
            ...
            # Forward（前向传播）
            with torch.autocast(
                device_type=device.type, enabled=cuda or device.type == "mps"
            ):
            ...
            if RANK in [-1, 0]:
                ...
                mem = (
                    f"{torch.cuda.memory_reserved() / 1E9 if cuda else 0:.3g}G"  # (GB)
                )
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default=ROOT / "pretrained/yolov5s.pt",
        help="initial weights path",
    )
    parser.add_argument(
        "--cfg", type=str, default=ROOT / "models/yolov5s.yaml", help="model.yaml path"
    )
    parser.add_argument(
        "--data",
        type=str,
        default=ROOT / "data/self_data.yaml",
    ...
    parser.add_argument(
        "--device", default="", help="cuda device, i.e. 0 or 0,1,2,3 or mps or cpu"
    )
    parser.add_argument(
    ...
    parser.add_argument("--root", default=None, help="GUI")
    parser.add_argument("--progressbar", default=None, help="progressbar")
