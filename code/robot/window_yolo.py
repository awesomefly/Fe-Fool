...
class YoloDataWindow:
    ...
    def window(self):
        ...
        self.label1 = tkinter.Label(self.root, text='请输入每张背景需要生成的数据量:', font=('黑体', 12))
        self.inp1 = tkinter.Entry(self.root)
        self.inp1.insert(0, "5")
        self.label1.grid(row=0, column=0)
        self.inp1.grid(row=0, column=1)

        self.label2 = tkinter.Label(self.root, text='请输入前景间重叠因子(越小随机的各个前景间可重合度越高):', font=('黑体', 12))
        self.inp2 = tkinter.Entry(self.root)
        self.inp2.insert(0, "1")
        self.label2.grid(row=1, column=0)
        self.inp2.grid(row=1, column=1)

        self.label3 = tkinter.Label(self.root, text='请输入每个前景在背景中的随机的最大个数:', font=('黑体', 12))
        self.inp3 = tkinter.Entry(self.root)
        self.inp3.insert(0, "3")
        self.label3.grid(row=2, column=0)
        self.inp3.grid(row=2, column=1)

        self.label4 = tkinter.Label(self.root, text='物品是否为圆形:', font=('黑体', 12))
    ...
    def run(self, per_num=0, overlap_factor=0, max_num=0, is_circle=True):
        ...
        image_generate_dataset.run(per_num=per_num, overlap_factor=overlap_factor, max_num=max_num, is_circle=is_circle, root=self.root, progressbar=progressbar)
